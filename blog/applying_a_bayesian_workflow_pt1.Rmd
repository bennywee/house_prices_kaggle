...
The no pooling or dummy variable regression is written out below, where i indexes the property and j indexes each neighbourhood. I've assigned a gaussian likelihood which assumes that the residuals are normally distributed. 
...
<!-- Potential question: Why different scaling? For y, using mean 0 and std 1. Index variable allows us to assign the same prior to all categories-->
...
We know that the variance parameter over $\sigma$ is defined over positive reals. So our prior should only put probabilistic weight on possible values. In this case I've chosen a Half normal distribution which has thinner tails than other candidate priors such as the exponential or half-cauchy distributions.
