...
Developments around "workflow" are a current topic of research. The most recent paper came out on November 2nd titled "Bayesian Workflow" which has many contributions from active members in the statistics community. This writeup was prepared before I had a chance to read the paper, but I hope it covers some of the principles and recommendations. And if not, like statistical models, I the way I do statistical modelling will iterate and improve. 
...
9) _Optional:_ Evaluate model performance on test or out of sample dataset (depends on your problem, is it a predictive task or a causal inference task?)
...
2) No pooling OLS (assumes independence between features)
3) Saturated regression (adds interactive effects between `log(LotArea)_i` and `neighbourhood` to no pooling OLS)

I will use the no pooling OLS model to demonstrate rest of the workflow. These models can certainly be improved on. In fact, this problem is a good candidate for _heirarchical_ or _multilevel_ models. This allows information to be borrowed between neighbourhood through partial pooling and removes the assumption of independence. This will be extremely useful given that the sample is imbalanced across `neighbourhood` and will help regularise the effects of small sample sizes. I will introduce this in the next post. 
...
# Scale target
df['log_sales_price_std'] = (df['log_sales_price'] - df['log_sales_price'].mean()) /df['log_sales_price']std()
...
We know that the variance parameter $\sigma$ is defined over positive reals. So our prior should only put probabilistic weight on possible values. In this case I've chosen an exponential(1) prior. Other candidate priors are the half-cauchy distribution or the half-normal which has thinner tails.

<!--Half normal distribution which has thinner tails than other candidate priors such as the exponential or half-cauchy distributions.-->
...
So in the case of $\alpha_j$ I set a normal prior with a mean of 0 and a standard deviaion of 1 for all neighbourhoods, regularising neighbourhood effects to be within two standard deviations of $y$. 

<!--This assumes that the average neighbourhood price is between 50\% and 150\% of the overall average property price with 95\% probability. -->

In summary, weakly regulating priors express my belief that the parameters of this model would overfit the sample and that we need to regularise their effects. Standardising the variables made this job much easier and intuitive. 

All together the full model looks like:
...
### 3)  Prior predictive checks - simulate fake data from the implied generative model{.tabset .tabset-fade .tabset-pills}

We want to perform prior predictive simulations to understand the implications of our priors and likelihood. This will help us diagnose any problems with our assumptions and modelling decisions. This works by simulating parameters from the joint prior distribution, _without_ evaluating the likelihood (i.e before the model is trained on the data). We can then see what the implied distribution of the outcome variable `y` is relative to the actual outcome. 

I've provided some heavily commented stan code below which shows how to specify the no pooling model and execute it via python.
...
# Estimate model by drawing samples from joint prior distribution
fit_npm_check = no_pooling_stan_code.sampling(data=npm_data_check)

# Extract sampled values into a pandas dataframe
...
The implied predictions of our priors are visualised below. It shows the implied linear association when `x` increases by one standard deviation with `y`. Since the priors for the neighbourhoods are the same, I've arbitrairily chosen the 4th neighbourhood indexed as the $\alpha$. Remember that ~95\% of gaussian mass exists within two standard deviations of the mean. So we should keep this in mind when choosing weakly informative priors to ensure that we create bounds to values which are possible while allowing relationships which are implausible. I've included an example of what happens if we choose extremely wide (uninformative) priors and its implications on the data generating process. 
...
Take one of the simulated blue lines (fake data) and the parameters sampled from the prior which correspond to that line (let's say the 50th simulation) and estimate the model. Let's see if our model fitted on fake data can capture the "true" parameters (dotted red lines) which generated the distribution. If your model cannot capture the _known_ parameters which generated fake data, there is no way it will be estimating the correct parameters on real data.
...
Now we want to check how well our model performed. We can perform posterior predictive checks to see if simulated distributions from the fitted model can successfully predict the observed training data. If the model performs well, it should be able to retrodict the density of the data used to train the model. The blue lines are the predictions drawn from the joint posterior distribution compared with the observed density of the $y$ target variable
...
We can transform this to get the posterior predictive checks on the natural scale (rescale $y$ and exponentiate `log(SalesPrice)` to get back `SalesPrice`:
...
Not too bad for a very simple model. We can definitely iterate and improve on this.

### Conclusion

The last things we should do is compare the fits of multiple models and evaluate their performance using cross validation. Since we only fitted a single model, I have reserved this for the next post which applies the complete workflow on more complex multilevel/heirarchical models. Since this is also a predictive task, we can also evaluate model performance on out of sample data (in this kaggle competition, we would evaluate the log RMSE of the out of sample dataset). 

Note that this is not an exhaustive review of all the diagnostics and visualiastions that can be performed in a workflow. There are multiple ways of evaluating model fit and performing prior/posterior predictive checks that could validate or invalidate the model. Hopefully you found this useful and I welcome any feedback!
