...
generated quantities {
    ...
    for(n in 1:N){
          mu[n] = alpha_j[neighbourhood[n]] + beta_j[neighbourhood[n]] * log_lot_area[n];
          log_lik[n] = normal_lpdf(log_sales_price[n] | mu[n], sigma);
          y_hat[n] = normal_rng(mu[n], sigma);      
        }
    for(n in 1:N_test){
          mu_test[n] = alpha_j[neighbourhood_test[n]] + beta_j[neighbourhood_test[n]] * log_lot_area_test[n];
