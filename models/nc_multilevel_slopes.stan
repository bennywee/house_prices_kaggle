...
model {
    ...
    for(n in 1:N) mu[n] = alpha_nh[n] + beta_nh[n] * log_lot_area[n];
...
generated quantities {
    ...
    for(n in 1:N){
          mu[n] = alpha_nh[n] + beta_nh[n] * log_lot_area[n];
          log_lik[n] = normal_lpdf(log_sales_price[n] | mu[n], sigma);
          y_hat[n] = normal_rng(mu[n], sigma);      
        }
    for(n in 1:N_test){
          mu_test[n] = alpha_nh_test[n] + beta_nh_test[n] * log_lot_area_test[n];
