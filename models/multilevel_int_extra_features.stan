...
model {
    ...
    // Likelihood
    if(run_estimation==1){
        target += normal_lpdf(log_sales_price | alpha_j[neighbourhood] + X * beta, sigma);
...
generated quantities {
    ...
    {
    for(n in 1:N){
          log_lik[n] = normal_lpdf(log_sales_price | alpha_j[neighbourhood[n]] + X[n,] * beta, sigma);
          y_hat[n] = normal_rng(alpha_j[neighbourhood[n]] + X[n,] * beta, sigma);      
        }
    for(n in 1:N_test){
          y_test[n] = normal_rng(alpha_j[neighbourhood_test[n]] + X_test[n,] * beta, sigma);
        }
    }
}